...
[As we saw earlier](https://github.com/BHoM/documentation/wiki/BH.oM-‐-Define-New-Objects#BHoM-object-functionality-constructors-and-methods), this structure gives us a few advantages, in particular:
...
But what if we do not have a method implemented for every type that that can be dynamically called by **IBounds**? That is what private fallback methods are for. In general fallback methods are used for handling unexpected behaviours of main method. In this case it should log an error with a proper message (see [Handling Exceptional Events](https://github.com/BHoM/documentation/wiki/Handling-Exceptional-Events) for more information) and return null or NaN.
...
[[/images/PolymorphicExtention_SpeedTest.png]] 
